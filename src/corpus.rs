...
fn main() {
    ...
    let tar = OpenOptions::new()
        ...
        .write(true)
        .create(true)
        .open("archives/server/intermediate.tar")
        .expect("Create intermediate tar");
    let mut a = Builder::new(tar);

    a.append_dir_all("", "corpus").unwrap();
    let mut a = a.into_inner().unwrap();
    let mut archive_data = String::new();
    a.seek(SeekFrom::Start(0)).unwrap();
    a.read_to_string(&mut archive_data).unwrap();
    ...
    let data_to_verify = archive_meta.clone() + &archive_data;
    ...
    fs::rename(
        "archives/server/intermediate.tar",
        format!("archives/server/{}{}", &archive_meta, &signature_string) + ".tar",
    ).unwrap();
}
